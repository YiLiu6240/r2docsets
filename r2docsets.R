...
replace.link.to.description <- function(doc) {
  ...
  nodes %>%
    lapply(function(x){
      oldNode <- newNode <- x
      xmlAttrs(newNode)["href"] <- ifelse(
        tools::file_ext(xmlAttrs(oldNode)["href"]) != "html",
        paste0(xmlAttrs(oldNode)["href"], ".html"), xmlAttrs(oldNode)["href"])
      replaceNodes(oldNode, newNode)
    })
}
...
list.of.dir <- function(pkg, path) {
  ...
  write(sprintf("<h1>Listing of directory %s</h1><hr><dl>",
                file.path("library", pkg, "doc")),
        path, append = TRUE)
  for (l in viganettes) {
    write(sprintf("<dt></dt><dd><a href=%s>%s</a></dd>", l, l),
          path, append = TRUE)
  }
...
generate.help.from.Rd <- function(pkg) {
  ...
  as.list(topics) %>%
    lapply(function(x){
      tools::Rd2HTML(pkgRdDB[[x]], out = file.path("library", pkg, "html",
                                                   paste(x, ".html", sep = "")),
                     package = pkg, Links = tools::findHTMLlinks())
    })
}
...
generate.help.html <- function(pkg) {
  ...
  file.copy(system.file('misc', 'R.css', package = 'knitr'),
            file.path("library", pkg, "html"))

  # 00index.html
  file.copy(file.path(find.package(pkg), 'html', "00Index.html"),
            file.path("library", pkg, "html"))
  doc <- htmlTreeParse(file.path("library", pkg, "html", "00Index.html"),
                       useInternal = TRUE)
  replace.logo(doc)
  remove.navigation(doc)
  ...
  c("DESCRIPTION", "NEWS") %>%
    lapply(function(x){
      if (file.exists(system.file(x, package = pkg))) {
        file.copy(system.file(x, package = pkg), f
                  <- file.path("library", pkg, x),
                  copy.date = TRUE)
  ...
  if (system.file("doc", package = pkg) != "") {
    file.copy(system.file("doc", package = pkg),
              file.path("library", pkg), recursive = TRUE, copy.date = TRUE)
    ## index.html
    if (file.exists(file.path("library", pkg, "doc", "index.html"))) {
      doc <- htmlTreeParse(file.path("library", pkg, "doc", "index.html"),
                           useInternal = TRUE)
      replace.logo(doc)
      remove.navigation(doc)
      getNodeSet(doc, "/html/head/link") %>%
        lapply(function(x){
          oldNode <- newNode <- x
...
create.sqlite.index <- function(pkg, conn) {
  ...
  for (n in nodes.tr) {
    ...
    dbGetQuery(conn,
               sprintf("insert into searchIndex(name, type, path, package, version) values('%s', '%s', '%s', '%s', '%s')",
                       name, type, path, pkg, packageVersion(pkg)))
  }
  dbGetQuery(conn,
             sprintf("insert into searchIndex(name, type, path, package, version) values('%s', 'Package', '%s', '%s', '%s')",
                     pkg, file.path("library", pkg, "html", "00Index.html"),
